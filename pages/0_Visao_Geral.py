...
from util.style import inject_css, metric_card, insight, section_header, PLOTLY_LAYOUT, COLORS, get_layout, HOVER
...
<div style="margin-bottom:2rem;">
    <div style="font-size:0.7rem;font-weight:600;letter-spacing:0.2em;text-transform:uppercase;color:#6EE7B7;margin-bottom:0.4rem;">
        DATATHON · FIAP PÓSTECH DATA ANALYTICS
    </div>
    <h1 style="font-size:2.4rem;font-weight:800;color:#F1F5F9;line-height:1.1;margin:0 0 0.5rem;">
        Luminar de Dados<br/>
        <span style="color:#6EE7B7;">Iluminando Vidas</span>
    </h1>
...
bolsista_pv = (df_b[df_b['BOLSISTA_2022']=='Sim']['PONTO_VIRADA_2022']=='Sim').mean()*100
topazio_22 = (df['PEDRA_2022']=='Topázio').sum()
topazio_20 = (df['PEDRA_2020']=='Topázio').sum()
...
with col_a:
    ...
    for _, row in sample.iterrows():
        ...
        line_color = 'rgba(110,231,183,0.15)' if is_up else 'rgba(248,113,113,0.15)'
    ...
    apply_layout(fig, height=340, yaxis_title="INDE", yaxis_range=[3,10],
    ...
    st.markdown(insight(
        ...
        "nas fases avançadas do programa.",
        "warning"
    ), unsafe_allow_html=True)
...
with col_b:
    ...
    for pedra in PEDRA_ORDER:
        ...
        fig2.add_trace(go.Bar(
            ...
            textposition='inside',
            textfont=dict(color='#0F1117', size=11, family='Inter'),
        ))

    apply_layout(fig2, height=240, barmode='stack', yaxis_title="%",
...
with col_c:
    st.markdown(section_header("03 — IMPACTO DO PROGRAMA DE BOLSAS","Bolsistas vs Escola Pública"), unsafe_allow_html=True)
    ...
    grupos['Label'] = grupos['BOLSISTA_2022'].map({'Sim':'Bolsistas','Não':'Escola Pública'})

    fig3 = go.Figure()
    fig3.add_trace(go.Bar(
        name='INDE Médio',
        x=grupos['Label'], y=grupos['INDE'],
    ...
    fig3.update_layout(**get_layout(height=300, barmode='group',
    ...
    st.markdown(insight(
        ...
        "Ponto de Virada</strong> (38,9% vs 9,4%). "
        "No Top 10% do INDE, 39,1% são bolsistas — contra 3,4% no Bottom 10%. "
        "Expandir bolsas é a intervenção com maior ROI educacional evidenciado.",
        "success"
    ), unsafe_allow_html=True)
...
with col_d:
    ...
    ips_q25 = df22['IPS_2022'].quantile(0.25)
    iaa_q25 = df22['IAA_2022'].quantile(0.25)
    ipp_q25 = df22['IPP_2022'].quantile(0.25)

    vuln_cats = {
        'IPS Psicossocial': (df22['IPS_2022'] < ips_q25).sum(),
        'IAA Autoavaliação': (df22['IAA_2022'] < iaa_q25).sum(),
        'IPP Psicopedagógico': (df22['IPP_2022'] < ipp_q25).sum(),
        'Multi-vulnerável\n(IPS + IAA)': ((df22['IPS_2022'] < ips_q25) & (df22['IAA_2022'] < iaa_q25)).sum(),
    }
    total_22 = len(df22)

    fig4 = go.Figure(go.Bar(
        x=list(vuln_cats.values()),
        y=list(vuln_cats.keys()),
        orientation='h',
        marker_color=[COLORS['accent2'], COLORS['accent2'], COLORS['accent2'], COLORS['red']],
        text=[f"{v} alunos ({v/total_22*100:.1f}%)" for v in vuln_cats.values()],
        textposition='outside',
        textfont=dict(color=COLORS['muted'], size=11),
    ))

    fig4.update_layout(**get_layout(height=220, xaxis_title="Nº de alunos",
                   xaxis_range=[0, max(vuln_cats.values())*1.4],
    ...
    st.markdown(insight(
        "<strong>73 alunos são multi-vulneráveis</strong> (IPS + IAA abaixo do percentil 25 simultaneamente) — "
        "baixo suporte psicossocial combinado com autoavaliação negativa. "
        "Esse grupo tem probabilidade de Ponto de Virada próxima de zero. "
        "Intervenção prioritária da equipe de Psicologia.",
        "danger"
    ), unsafe_allow_html=True)
...
with col_e:
    colors_bar = [COLORS['accent'] if i == 0 else
                  COLORS['red'] if i == 1 else
                  COLORS['accent2'] for i in range(len(top_motivos))]

    fig5 = go.Figure(go.Bar(
        x=top_motivos['Total'],
        y=top_motivos['Motivo'],
        orientation='h',
    ...
    apply_layout(fig5, height=300, xaxis_title="Número de evasões",
                   xaxis_range=[0, top_motivos['Total'].max()*1.35],
                   margin=dict(l=16,r=80,t=10,b=16),
...
with col_f:
    st.markdown(insight(
        "<strong>Transferência (18,8%) e Sem Retorno ao Contato (17,6%)</strong> "
        "são os dois maiores motivos de evasão — somam 36% do total. "
        "Ambos indicam ruptura de vínculo, não rejeição ativa ao programa.",
        "info"
    ), unsafe_allow_html=True)
    st.markdown(insight(
        "<strong>A hipótese de evasão masculina por trabalho não se confirma.</strong> "
        "Taxa de evasão: F=53% vs M=57% pós-2020 — diferença de 4 p.p. sem relevância. "
        "Motivo 'Abandono' (que pode incluir trabalho): 41% feminino vs 59% masculino.",
        "warning"
    ), unsafe_allow_html=True)
...
labels = ['Desempenho\nAcadêmico','Engajamento','Ponto de\nVirada',
          'Autoavaliação','Psicossocial','Psicopedagógico','Nível de\nAprendizado']
...
fig6.add_trace(go.Bar(
    ...
    opacity=0.85,
    yaxis='y',
    text=[f"{p}%" for p in pesos],
    textposition='inside',
    textfont=dict(color='#0F1117', size=12),
))
fig6.add_trace(go.Scatter(
    name='Correlação com INDE',
    x=inds, y=corrs,
    mode='lines+markers',
...
fig6.update_layout(**get_layout(height=300,
    yaxis=dict(**PLOTLY_LAYOUT['yaxis'], title='Peso (%)', range=[0,28]),
    yaxis2=dict(overlaying='y', side='right', title='Correlação de Pearson',
...
st.markdown(insight(
    ...
    "IPS e IPP têm peso igual (10%) mas correlação muito baixa (~0,28) — "
    "medem dimensões importantes do aluno mas não se traduzem diretamente no índice numérico. "
    "<strong>INDE = IAN×10% + IDA×20% + IEG×20% + IAA×10% + IPS×10% + IPP×10% + IPV×20%</strong> "
    "(verificado com R²=1,000 e erro=0 nos dados).",
    "info"
), unsafe_allow_html=True)
